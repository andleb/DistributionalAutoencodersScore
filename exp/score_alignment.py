...
warnings.filterwarnings(
    "ignore", category=mpl._api.deprecation.MatplotlibDeprecationWarning
)
...
def _gradient_norm(
    encoder: torch.nn.Module, xy: np.ndarray, component: int | None, device="cpu"
) -> np.ndarray:
...
def _collect_levelset_stats(
    encoder, density_fn, limits, grid_res, component, device, atol=1e-5
):
    nx, ny = grid_res
    ...
    pts = torch.tensor(
        np.stack([xv.ravel(), yv.ravel()], 1), dtype=torch.float32, device=device
    )
    with torch.no_grad():
        z_val = encoder(pts)
        if component is not None:
    ...
    levels = np.asarray(
        [levels[0], *[l for l in levels[1:] if abs(l - levels[0]) > atol]]
    )
    stats = {}
    for L in levels:
        ...
        for col in cs.collections:
            for path in col.get_paths():
                verts = path.vertices
                st = levelset_stats(
                    encoder, density_fn, verts, component=component, device=device
                )
                if st["mass"] < _TINY:
                    continue
                if L in stats:
...
def cosine_alignment_theorem2p6(
    encoder,
    score_fn,
    density_fn,
    limits,
    ...
    stats = _collect_levelset_stats(
        encoder, density_fn, limits, grid_res, component, device=device
    )
    nx, ny = grid_res
    xs = np.linspace(limits[0][0], limits[0][1], nx)
    ys = np.linspace(limits[1][0], limits[1][1], ny)
...
def available_ckpts(prefix):
    return sorted(
        int(m.group(1))
        for m in (
            re.search(r"model_(\d+)\.pt", p) for p in glob.glob(f"{prefix}/model_*.pt")
        )
    ...
    for ep in available_ckpts(prefix):
        ...
        for comp in (0, 1):
            for ign in (False, True):
                mean, std, p95, *_ = cosine_alignment_theorem2p6(
                    enc,
                    score_fn,
                    pdf_fn,
                    limits,
...
if __name__ == "__main__":
    ...
    best_md_gauss = eval_family(
        label="Gaussian",
        prefix="res/models/gaussian/1k3l256h_old",
        pdf_fn=gaussian_pdf,
        score_fn=gaussian_score,
        limits=((-4, 4), (-4, 4)),
        grid_res=100,
        device=device,
    ...
    best_md_trim = eval_family(
        label="Trimodal",
        prefix="res/models/trimodal/1k3l256h",
        pdf_fn=gmm_pdf,
        score_fn=gmm_score,
        limits=((-4, 4), (-4, 4)),
        grid_res=100,
